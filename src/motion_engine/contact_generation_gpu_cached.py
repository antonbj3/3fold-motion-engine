"""Cached first-query neighbours emit canonical contacts and fused keys.
...
def build_kernels(wp):
    ...
    @wp.kernel
    def emit(p: wp.array(dtype=wp.vec3), owner: wp.array(dtype=int), records: wp.array2d(dtype=int),
             counts: wp.array(dtype=int), offsets: wp.array(dtype=int), stride: wp.int64, body_stride: wp.int64,
        ...
        if xi[2]-_R < 0.0:
            n -= 1
        for slot in range(n):
            candidates[slot] = records[slot, i]
...
def generate(points, owners):
    ...
    records = wp.empty((32, len(p)), dtype=int, device="cuda:0")
    wp.launch(count, len(p), inputs=[allp, owner, grid.id, counts, maximum, records], device="cuda:0")
    ...
    wp.launch(emit, len(p), inputs=[allp, owner, records, counts, offsets,
...
if __name__ == "__main__":
    ...
    runpy.run_path(str(Path(__file__).resolve().parents[2]/"probes"/"innovation_contact_gpu_cached_probe.py"), run_name="__main__")
