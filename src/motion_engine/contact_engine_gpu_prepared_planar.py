"""Reuse immutable planar coordinates within each pair-selection call."""
from motion_engine.contact_engine_gpu_colored import _MAXSEG, _MAXC
from motion_engine.contact_engine_gpu_pair_layout_cache import PairLayoutCacheContactEngine


def build_prepared_select(wp, max_keep=8):
    ...
    def k_select(C: int, mx: int, leaders: wp.array(dtype=int), leader_scan: wp.array(dtype=int), coords: wp.array(dtype=wp.vec2d), gorder: wp.array(dtype=int), flag: wp.array(dtype=int),
        ...
        for t in range(L):
            g = gorder[i + t]
            coords[g] = _planar(cpA[g], ref, u, w)
        rd = coords[gd]
...
class PreparedPlanarContactEngine(PairLayoutCacheContactEngine):
    def __init__(self,trim_unused_priorities=True,**kwargs):
        super().__init__(**kwargs)
        self.trim_unused_priorities=bool(trim_unused_priorities)
        self._packed_select=build_prepared_select(self.wp)

    def _build(self):
        super()._build()
        self._planar_coords=self.wp.empty(_MAXC,dtype=self.wp.vec2d,device=self.dev)
...
if __name__=='__main__':
    ...
    runpy.run_path(str(Path(__file__).resolve().parents[2]/'probes/innovation_prepared_planar.py'),run_name='__main__')
