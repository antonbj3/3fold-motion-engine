"""Pair solver variant with a consistent normal-derived tangent frame."""
from motion_engine.contact_engine_gpu_colored import GraphColoredContactEngine


def build_fixed_frame(wp):
    @wp.kernel
    def k_fixed_frame(porder: wp.array(dtype=int), pidx: wp.array(dtype=int), pstart: wp.array(dtype=int),
    ...
    return k_fixed_frame


class FixedFrameContactEngine(GraphColoredContactEngine):
    def __init__(self, **kwargs):
        kwargs.update(manifold_reduce=True, graph_capture=True, pair_chunks=True,
                      chunk_mode="registers")
        super().__init__(**kwargs)
        self.KC = dict(self.KC)
        self.KC["chunk_vel_r"] = build_fixed_frame(self.wp)
...
if __name__ == "__main__":
    ...
    runpy.run_path(str(Path(__file__).resolve().parents[2] /
                      "probes/innovation_stack_fixed_frame.py"), run_name="__main__")
