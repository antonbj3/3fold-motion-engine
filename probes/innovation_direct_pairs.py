"""Certify direct GPU body-pair manifold integration against full baseline traces."""
...
def worker(reverse=False):
    from motion_engine.contact_engine_gpu_direct_pairs import DirectPairsContactEngine
    ...
    constructors=[('baseline',PreparedPlanarContactEngine),('direct_pairs',DirectPairsContactEngine)]
    rows={}
...
def main():
    ...
    (ROOT/'reports/innovation_direct_pairs.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(candidates),flush=True)
    return 0 if all(candidates['direct_pairs']['gates'].values()) else 1
