...
import numpy as np
from cross_hardware_state_probe import ENGINES, SCENES, fingerprint
...
def matrix(directory, labels):
    ...
    for label in labels:
        ...
        report = json.loads((directory / f'cross_hardware_{label}.json').read_text())
        archive = directory / f'cross_hardware_{label}.npz'
    ...
    return dict(scope='Contact fixtures only; other suite families remain pending', hardware=hardware, rows=rows)
...
def main():
    ...
    (args.reports / 'cross_hardware_matrix.json').write_text(json.dumps(first, indent=2) + '\n')
    (args.reports / 'cross_hardware_matrix.md').write_text(render(first))
