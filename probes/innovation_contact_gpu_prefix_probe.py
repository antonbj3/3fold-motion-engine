"""Compare the frozen canonical prefix with the unchanged cached generator."""
...
def worker():
    ...
    from motion_engine.contact_generation_gpu_cached import build_kernels
    ...
    for bodies in (1000,10000):
        ...
        o=e.owner.numpy()
        n=len(p)
        records=wp.empty((32,n),dtype=int,device=e.dev)
        ...
        def baseline():
            ...
            wp.launch(e.KD["fkey"],c,inputs=[c,e.cfa,e.cfb,n+1,e.skey,e.sval],device=e.dev)
            wp.utils.radix_sort_pairs(e.skey,e.sval,c)
            wp.copy(e.order,e.sval,count=c)
            wp.launch(e.KC["gather"],c,inputs=[e.order,c,*ref_fields[:6],
                      e.tbi,e.tbj,e.tpA,e.tpB,e.tn,e.tpen],device=e.dev)
            wp.synchronize()
        def candidate():
            e.grid.build(e.allp,2*_R)
            maximum.zero_()
            wp.launch(count,n,inputs=[e.allp,e.owner,e.grid.id,counts,maximum,records],device=e.dev)
            ...
            wp.launch(emit,n,inputs=[e.allp,e.owner,records,counts,offsets,wp.int64(n+1),
                      wp.int64(bodies+1),*fields],device=e.dev)
            latest["candidate_count"],latest["max_neighbours"]=c,mx
            wp.synchronize()
        timings={}
        for name,fn in (("baseline_canonical_prefix",baseline),("candidate_canonical",candidate)):
            for _ in range(10):
                fn()
        ...
        prefix=[a.numpy()[:c] for a in (e.tbi,e.tbj,e.tpA,e.tpB,e.tn,e.tpen,e.skey)]
        expected_prefix=ref[:6]+[ref[-2]]
        prefix_exact=all(a.dtype==b.dtype and a.shape==b.shape and a.tobytes()==b.tobytes()
                         for a,b in zip(prefix,expected_prefix))
        equal=all(a.dtype==b.dtype and a.shape==b.shape and a.tobytes()==b.tobytes() for a,b in zip(ref,got))
        results.append({"bodies":bodies,"points":n,"cache_bytes":32*n*4,**latest,"timings_ms":timings,
                        "exact_reference":equal,"prefix_exact":prefix_exact,"candidate_sha256":hashlib.sha256(b"".join(a.tobytes() for a in got)).hexdigest(),
...
def main():
    ...
    gates={"exact_reference":all(r["exact_reference"] for leg in legs for r in leg),
           "prefix_exact":all(r["prefix_exact"] for leg in legs for r in leg),
           ...
           "no_slower_than_canonical_prefix":all(r["timings_ms"]["candidate_canonical"]<=r["timings_ms"]["baseline_canonical_prefix"] for leg in legs for r in leg)}
    report={"legs":legs,"backgrounds":backgrounds,"gates":gates,"warm":10,"timed":30,
            "scope":"frozen raw generation plus feature sort/gather versus cached canonical generation; excludes downstream pair grouping"}
    (ROOT/"reports"/"innovation_contact_gpu_prefix.json").write_text(json.dumps(report,indent=2)+"\n")
