...
def worker(profile, output):
    ...
    e=ColorCacheContactEngine(dims=DIMS,mu=.5,vit=40,pit=20,profile=profile)
    ...
    print('RESULT '+json.dumps(dict(profile=profile,wall_ms=wall,phases=phases,
        phase_sum_ms=sum(phases.values()),outside_phase_ms=wall-sum(phases.values()),
        contacts=e.n_contacts,selected=e.n_contacts_solved,pairs=e.n_pairs,colors=e.n_colors,
        solve_nodes=e.n_colors*(1+e.vit+e.pit),cache_hits=e.color_cache_hits,cache_misses=e.color_cache_misses,
...
def main():
    ...
    gates=dict(exact_full_states=exact,finite=all(r['finite'] and np.isfinite(r['wall_ms']) and r['wall_ms']>0 for r in rows),
    ...
    np.savez_compressed(ROOT/'reports/innovation_color_cache_profile_v2.npz',**{mode+'__'+k:a for mode,values in arrays.items() for k,a in values.items()})
    (ROOT/'reports/innovation_color_cache_profile_v2.json').write_text(json.dumps(dict(rows=rows,gates=gates,background=background),indent=2)+'\n')
