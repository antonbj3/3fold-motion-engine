...
def worker(profile, output):
    ...
    observed=ColorCacheContactEngine(dims=DIMS,mu=.5,vit=40,pit=20)
...
def main():
    ...
    np.savez_compressed(ROOT/'reports/innovation_island_profile_v1.npz',**{mode+'__'+k:a for mode,values in arrays.items() for k,a in values.items()})
    (ROOT/'reports/innovation_island_profile_v1.json').write_text(json.dumps(dict(rows=rows,gates=gates,background=background),indent=2)+'\n')
