"""Separate split cone shadow solver with unchanged objective and acceptance gates."""
...
def solve(s):
    ...
    energy0=objective(x)
    rho=lipschitz/1.0000001*.1
    factor=cho_factor(gram+rho*np.eye(3*c),lower=True,check_finite=False)
    dual=np.zeros_like(x)
    residual=float('inf')
    for iteration in range(1,40001):
        unconstrained=cho_solve(factor,rho*(x-dual)-linear,check_finite=False)
        x=project(unconstrained+dual)
        dual+=unconstrained-x
    ...
    return {'call':s['call'],'contacts':c,'rho':rho,'iterations':iteration,'projected_residual':residual,
...
def main():
    ...
    (ROOT/'reports/innovation_stack_cone_split_shadow.json').write_text(json.dumps(report,indent=2)+'\n')
