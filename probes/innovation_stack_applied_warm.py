"""Two independent applied-warm full-engine sweeps with unchanged gates."""
...
def worker():
    ...
    for budget in (20, 40):
        ...
        rows[str(budget)] = dict(m5=m5, m1=m1, trajectory_sha256=observer.digest.hexdigest(),
...
def main():
    ...
    for budget in ("20", "40"):
        ...
        keys = ("m5", "m1", "trajectory_sha256", "large_state_sha256", "force_error", "finite")
        gates[budget] = dict(exact_repeats=all(rows[0][k] == rows[1][k] for k in keys),
    ...
    report = dict(legs=legs, backgrounds=backgrounds, gates=gates,
                  day_plan_speed={k: all(r[k]["timing"]["ms_per_step"] <= 4.9 for r in legs)
                                  for k in gates})
    (ROOT / "reports/innovation_stack_applied_warm.json").write_text(json.dumps(report, indent=2) + "\n")
