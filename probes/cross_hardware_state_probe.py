...
from outclass_rescore_probe import factories
...
def worker(engine,scene,output):
    ...
    result=dict(engine=engine,scene=scene,initial=initial,arrays={k:fingerprint(v) for k,v in arrays.items()},
...
def capture(label):
    ...
    archive=ROOT/f'reports/cross_hardware_{label}.npz'
    ...
    (ROOT/f'reports/cross_hardware_{label}.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(gates),flush=True)
    return 0 if all(gates.values()) else 1


def compare(labels):
    reports={label:json.loads((ROOT/f'reports/cross_hardware_{label}.json').read_text()) for label in labels}
    comparisons=[]
    base=labels[0]
    with np.load(ROOT/f'reports/cross_hardware_{base}.npz') as ref:
        for label in labels[1:]:
            diffs=[]
            with np.load(ROOT/f'reports/cross_hardware_{label}.npz') as other:
