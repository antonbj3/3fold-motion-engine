...
def worker(reverse=False):
    ...
    from motion_engine.contact_engine_gpu_unfused_solve import UnfusedColorCacheContactEngine
    from engine_metrics import DIMS,m5_penetration,m1_stack_load
    from colored_vs_jacobi import lattice,timed_steps
    constructors=[('baseline',ColorCacheContactEngine),('unfused',UnfusedColorCacheContactEngine)]
    rows={}
...
def main():
    ...
    (ROOT/'reports/innovation_unfused_physics.json').write_text(json.dumps(report,indent=2)+'\n')
