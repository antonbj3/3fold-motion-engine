"""Certify original physical gates for the noncontracting ColorCache candidate."""
...
def worker(reverse=False, prepared=False, island=False, compact=False, shared=False):
    from motion_engine.contact_engine_gpu_color_cache import ColorCacheContactEngine
    from motion_engine.contact_engine_gpu_unfused_position import UnfusedPositionColorCacheContactEngine
    from engine_metrics import DIMS,m5_penetration,m1_stack_load
    from colored_vs_jacobi import lattice,timed_steps
    constructors=[('baseline',ColorCacheContactEngine),('unfused',UnfusedPositionColorCacheContactEngine)]
    if prepared:
        from motion_engine.contact_engine_gpu_prepared_noncontracting import PreparedNoncontractingContactEngine
        constructors=[('baseline',UnfusedPositionColorCacheContactEngine),('unfused',PreparedNoncontractingContactEngine)]
        if island:
            from motion_engine.contact_engine_gpu_noncontracting_island import NoncontractingIslandContactEngine
            constructors=[('baseline',PreparedNoncontractingContactEngine),('unfused',NoncontractingIslandContactEngine)]
            if compact:
                from motion_engine.contact_engine_gpu_compact_island import CompactIslandContactEngine
                constructors=[('baseline',NoncontractingIslandContactEngine),('unfused',CompactIslandContactEngine)]
                if shared:
                    from motion_engine.contact_engine_gpu_shared_layout import SharedLayoutContactEngine
                    constructors=[('baseline',CompactIslandContactEngine),('unfused',SharedLayoutContactEngine)]
    rows={}
...
def main():
    ...
    compact='--compact' in sys.argv or shared
    island='--island' in sys.argv or compact
    prepared='--prepared' in sys.argv or island
    if len(sys.argv) in (3,4) and sys.argv[1]=='--worker':worker(sys.argv[2]=='1',prepared,island,compact,shared);return 0
    legs=[];background=[]
    for leg in range(2):
        before=idle()
        p=subprocess.run([sys.executable,__file__,'--worker',str(leg)]+(['--shared'] if shared else ['--compact'] if compact else ['--island'] if island else ['--prepared'] if prepared else []),cwd=ROOT,capture_output=True,text=True)
    ...
    candidates={}
    for name in ('unfused',):
        gates=dict(exact_repeats=all(numeric(legs[0][k])==numeric(legs[1][k]) for k in ('baseline',name)),
            ...
            coverage=all(len(r[name]['traces'])==3 and r[name]['timing']['bodies']==10000 for r in legs),
            original_speed=all(r[name]['timing']['ms_per_step']<=4 for r in legs))
        if prepared:
            gates['baseline_parity']=all(numeric(r['baseline'])==numeric(r[name]) for r in legs)
            gates['paired_speed']=all(r[name]['timing']['ms_per_step']<r['baseline']['timing']['ms_per_step'] for r in legs)
    ...
    filename='shared_layout.json' if shared else 'compact_island.json' if compact else 'noncontracting_island.json' if island else 'prepared_noncontracting.json' if prepared else 'innovation_position_physics.json'
    (ROOT/'reports'/filename).write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(candidates),flush=True)
    return 0 if all(candidates['unfused']['gates'].values()) else 1
