"""Locate the first differing solve kernel with frozen-contract parity."""
...
def main(label):
    ...
    np.savez_compressed(ROOT/f'reports/cross_kernel_{label}.npz',**all_arrays)
    (ROOT/f'reports/cross_kernel_{label}.json').write_text(json.dumps(dict(rows=rows,gates=gates),indent=2)+'\n')
