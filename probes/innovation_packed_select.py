"""Certify packed selection against frozen full-engine physical and byte gates."""
...
def worker(reverse=False):
    ...
    constructors=[('baseline',ColorCacheContactEngine),('packed',PackedSelectContactEngine)]
    rows={}
...
def main():
    ...
    gates=dict(exact_reference=all(numeric(r['baseline'])==numeric(r['packed']) for r in legs),
        exact_repeats=all(numeric(legs[0][name])==numeric(legs[1][name]) for name in ('baseline','packed')),
        k16=all(r['packed']['m5']['pass'] for r in legs),k4=all(r['packed']['m1']['pass'] for r in legs),
        rest=all(r['packed']['force_error']<.01 for r in legs),finite=all(r['packed']['finite'] for r in legs),
        no_timing_regression=all(r['packed']['timing']['ms_per_step']<=r['baseline']['timing']['ms_per_step'] for r in legs),
        original_speed=all(r['packed']['timing']['ms_per_step']<=4 for r in legs))
    report=dict(legs=legs,background=background,gates=gates,
                submillisecond_goal=all(r['packed']['timing']['ms_per_step']<1 for r in legs))
    (ROOT/'reports/innovation_packed_select.json').write_text(json.dumps(report,indent=2)+'\n')
