...
def worker(reverse=False):
    from motion_engine.contact_engine_gpu_packed_select import PackedSelectContactEngine as ColorCacheContactEngine
    from motion_engine.contact_engine_gpu_island_solve import IslandSolveContactEngine as PackedSelectContactEngine
    from engine_metrics import DIMS,m5_penetration,m1_stack_load
    from colored_vs_jacobi import lattice,timed_steps
    constructors=[('baseline',ColorCacheContactEngine),('island',PackedSelectContactEngine)]
    rows={}
...
def main():
    ...
    gates=dict(exact_reference=all(numeric(r['baseline'])==numeric(r['island']) for r in legs),
        exact_repeats=all(numeric(legs[0][name])==numeric(legs[1][name]) for name in ('baseline','island')),
        k16=all(r['island']['m5']['pass'] for r in legs),k4=all(r['island']['m1']['pass'] for r in legs),
        rest=all(r['island']['force_error']<.01 for r in legs),finite=all(r['island']['finite'] for r in legs),
        no_timing_regression=all(r['island']['timing']['ms_per_step']<=r['baseline']['timing']['ms_per_step'] for r in legs),
        original_speed=all(r['island']['timing']['ms_per_step']<=4 for r in legs))
    report=dict(legs=legs,background=background,gates=gates,
                submillisecond_goal=all(r['island']['timing']['ms_per_step']<1 for r in legs))
    (ROOT/'reports/innovation_island_solve.json').write_text(json.dumps(report,indent=2)+'\n')
