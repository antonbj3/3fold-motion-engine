...
def main(label):
    ...
    np.savez_compressed(ROOT/f'reports/cross_phase_{label}.npz',**all_arrays)
    (ROOT/f'reports/cross_phase_{label}.json').write_text(json.dumps(dict(rows=rows,gates=gates),indent=2)+'\n')
