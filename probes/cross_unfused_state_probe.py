...
def factories():
    ...
    out['pair_chunks']=lambda **kw:UnfusedPairContactEngine(dims=DIMS,mu=.5,manifold_reduce=True,graph_capture=True,pair_chunks=True,**kw)
    out['color_cache']=lambda **kw:UnfusedColorCacheContactEngine(dims=DIMS,mu=.5,**kw)
    return out
...
def worker(engine,scene,output):
    ...
    result=dict(variant="unfused_velocity_solve",engine=engine,scene=scene,initial=initial,arrays={k:fingerprint(v) for k,v in arrays.items()},
...
def capture(label):
    ...
    archive=ROOT/f'reports/cross_unfused_{label}.npz'
    ...
    (ROOT/f'reports/cross_unfused_{label}.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(gates),flush=True)
    return 0 if all(gates.values()) else 1


def compare(labels):
    reports={label:json.loads((ROOT/f'reports/cross_unfused_{label}.json').read_text()) for label in labels}
    comparisons=[]
    base=labels[0]
    with np.load(ROOT/f'reports/cross_unfused_{base}.npz') as ref:
        for label in labels[1:]:
            diffs=[]
            with np.load(ROOT/f'reports/cross_unfused_{label}.npz') as other:
