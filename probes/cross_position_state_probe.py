...
def factories():
    ...
    from motion_engine.contact_engine_gpu_unfused_position import UnfusedPositionPairContactEngine,UnfusedPositionColorCacheContactEngine
    out=original_factories()
    out['pair_chunks']=lambda **kw:UnfusedPositionPairContactEngine(dims=DIMS,mu=.5,manifold_reduce=True,graph_capture=True,pair_chunks=True,**kw)
    out['color_cache']=lambda **kw:UnfusedPositionColorCacheContactEngine(dims=DIMS,mu=.5,**kw)
    if '--prepared' in sys.argv or '--island' in sys.argv or '--compact' in sys.argv or '--shared' in sys.argv:
        from motion_engine.contact_engine_gpu_prepared_noncontracting import PreparedNoncontractingContactEngine
        out['color_cache']=lambda **kw:PreparedNoncontractingContactEngine(dims=DIMS,mu=.5,**kw)
        if '--island' in sys.argv or '--compact' in sys.argv or '--shared' in sys.argv:
            from motion_engine.contact_engine_gpu_noncontracting_island import NoncontractingIslandContactEngine
            out['color_cache']=lambda **kw:NoncontractingIslandContactEngine(dims=DIMS,mu=.5,**kw)
            if '--compact' in sys.argv or '--shared' in sys.argv:
                from motion_engine.contact_engine_gpu_compact_island import CompactIslandContactEngine
                out['color_cache']=lambda **kw:CompactIslandContactEngine(dims=DIMS,mu=.5,**kw)
                if '--shared' in sys.argv:
                    from motion_engine.contact_engine_gpu_shared_layout import SharedLayoutContactEngine
                    out['color_cache']=lambda **kw:SharedLayoutContactEngine(dims=DIMS,mu=.5,**kw)
    return out
...
def worker(engine,scene,output):
    ...
    result=dict(variant="unfused_velocity_position",engine=engine,scene=scene,initial=initial,arrays={k:fingerprint(v) for k,v in arrays.items()},
...
def capture(label):
        ...
        for scene in SCENES:
            ...
            with tempfile.TemporaryDirectory(prefix='state-capture-') as tmp:
                for leg in range(2):
                    ...
                    p=subprocess.run([sys.executable,str(Path(__file__).resolve()),'--worker',engine,scene,str(path)]+(['--shared'] if '--shared' in sys.argv else ['--compact'] if '--compact' in sys.argv else ['--island'] if '--island' in sys.argv else ['--prepared'] if '--prepared' in sys.argv else []),cwd=ROOT,capture_output=True,text=True)
    ...
    archive=ROOT/f'reports/cross_position_{label}.npz'
    ...
    (ROOT/f'reports/cross_position_{label}.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(gates),flush=True)
    return 0 if all(gates.values()) else 1


def compare(labels):
    reports={label:json.loads((ROOT/f'reports/cross_position_{label}.json').read_text()) for label in labels}
    comparisons=[]
    base=labels[0]
    with np.load(ROOT/f'reports/cross_position_{base}.npz') as ref:
        for label in labels[1:]:
            diffs=[]
            with np.load(ROOT/f'reports/cross_position_{label}.npz') as other:
...
if __name__=='__main__':
    if len(sys.argv) in (5,6) and sys.argv[1]=='--worker':worker(*sys.argv[2:5]);raise SystemExit(0)
    if len(sys.argv) in (3,4) and sys.argv[1]=='--capture':raise SystemExit(capture(sys.argv[2]))
