#!/usr/bin/env python3
"""Batch-adjoint -- B independent stance scenes solved in ONE launch sequence, with the adjoint.
...
WHAT IS COPIED FROM WHERE.  contact operator kernel is NOT imported (the external operator source is
not bundled); the kernels below are the contact operator study formulas rewritten with an env index, and
parity_gpu.py's dense-M^-1 adaptation (k_jt/k_mscat/k_acc2w/k_gath) rewritten the same way.
Nothing in contact operator study or parity is touched.
...
1.  rho per env: the BATCHED DENSE 12x12 G, not Lanczos.  n_c = 4 makes G_e a 12x12
    ...
    precisely the quantity batch-adjoint found the all-stick regime needs.  At n_c >= 32 the choice
    flips and the Lanczos/LOBPCG estimator of contact operator study §lattice_bench is the right one; the operator
    here (`_matvec`) is already the one it would need.

2.  rho from the REGIME, per env (regime-rule / batch-adjoint's feedback).  batch-adjoint: rho = lambda_min+ wins
...
3.  The gradient is grad_affine (batch-adjoint §7), batched.  The de Saxce sigma map is AFFINE, so
    ...
    -> 3 calls, exactly batch-adjoint's number.  The (I - B) sigma = a solve is n_slip x n_slip with
...
def k_bilat(M: int, C: int, G: wp.array(dtype=f64), A: wp.array(dtype=f64),
    ...
    satisfies the NCP's KKT conditions exactly, i.e. the env is all-stick and batch-adjoint says
    rho = lambda_min+.  Otherwise the cone (or the sign condition) binds and batch-adjoint says
...
@wp.kernel
def k_zero32(acc: wp.array(dtype=wp.float32)):
    acc[wp.tid()] = wp.float32(0.0)


@wp.kernel
def k_mscat32(C: int, NV: int, Mi: wp.array(dtype=f64), yt: wp.array(dtype=f64),
              scal: wp.array(dtype=f64), acc: wp.array(dtype=wp.float32)):
    e, c, k = wp.tid()
    s = f64(0.0)
    for l in range(NV):
        s = s + Mi[(e * NV + k) * NV + l] * yt[(e * C + c) * NV + l]
    wp.atomic_add(acc, e * NV + k, wp.float32(s * scal[e]))


@wp.kernel
def k_acc2w32(NV: int, acc: wp.array(dtype=wp.float32), scal: wp.array(dtype=f64),
...
def k_zth(Z: wp.array(dtype=mat33d), Hz: wp.array(dtype=vec3d), o: wp.array(dtype=vec3d)):
    """o = Z^T Hz, with NO D term -- parity did this subtraction on the host; here it never
...
def k_setsig(C: int, j: int, sidx: wp.array(dtype=int), nslip: wp.array(dtype=int),
    ...
    for c in range(C):
        sig[e * C + c] = f64(0.0)
    # run: the j >= 0 half of the guard was missing.  grad_affine calls this kernel with
    # the SENTINEL j = -1 for the "sigma = 0" call (batch_adjoint_gpu.py:1036); -1 < nslip[e] is
...
class BatchProxADMMGPU:
    ...
    def set_rho(self, mode):
        """regime  = regime-rule/batch-adjoint's rule with the regime read from the BILATERAL test
    ...
    def _matvec(self, xin, xout, use_rho):
        ...
        self.L(k_zero32, B_ * NV, [self.acc32])
        self.L(k_jt, (B_, C, NV), [C, NV, M, self.J, xin, self.yt])
        self.L(k_mscat32, (B_, C, NV), [C, NV, self.Mi, self.yt, self.scal, self.acc32])
        self.L(k_acc2w32, (B_, NV), [NV, self.acc32, self.scal, self.wv])
