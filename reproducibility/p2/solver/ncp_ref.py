...
Deliverable A of SPEC.md (same directory).
...
complementarity condition (case split shown here):
...
# ----------------------------------------------------------------------------
# Contract: Scene dataclass (SPEC.md "Rules": the A<->B contract).
...
def classify(lam, G, b, mu, tol_lam=1e-10, tol_rel=1e-6):
    ...
    labels = np.empty(n_c, dtype="<early_study")
    for c in range(n_c):
...
def _pgs(G, b, mu, iters, tol, warm, desax):
    ...
    if n_c == 0:
        return lam, np.zeros(0), np.empty(0, dtype="<early_study")
    rho = _rho_of(G, n_c)
...
def _admm_core(G, b, mu, iters, tol, warm, rho, adapt_every, desax):
    ...
    n = 3 * n_c
    if n_c == 0:
        return np.zeros(0), np.zeros(0), np.empty(0, dtype="<early_study")
    ...
    # scenes (historical mass-ratio-100 stack observation; supporting trace not bundled).
    if warm is None:
        gamma = np.zeros(n)
    else:
...
def solve_ncp_admm(G, b, mu, iters=5000, tol=1e-12, warm=None, rho=None,
    ...
    HISTORICAL NEGATIVE (supporting trace not bundled): a warm lam from the previous time step can
...
def _clip_poly(poly, plane_n, plane_d, eps=1e-9):
    ...
    mass-ratio-100 stack lost a corner at step 5; supporting trace not bundled)."""
    out = []
    m = len(poly)
    for i in range(m):
...
def scene_massratio_stack(ratio=1000.0, H=0.20, dt=1.0 / 240.0, mu=0.5, n=3):
    ...
    CADtoSIMReady/scripts/physics_exp/primal_vs_dual_massratio.py: H=0.20,
...
def scene_tower(n=8, H=0.20, dt=0.004, mu=0.5, mass=1.0):
    """(iv) 8-box tower eight_box_tower."""
    h = H / 2.0
